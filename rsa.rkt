  ...
  (lambda ((lst <list>) (key <key>))
    (let ((n (key-modulus key)))
      (let loop ((rst lst) (prev 0))
        (cond ((null? rst) rst)
              (else
               (cons (modulo (- (rsa-transform (car rst) key) prev) n)
                     (loop (cdr rst) (car rst)))))))))
      
(define encrypt-and-sign
  (lambda ((msg <string>) (public <key>) (private <key>))
    (let ((cur (rsa-encrypt msg public)))
     (cons
      cur
      (rsa-transform (compress cur public) private)))))

(define decrypt-and-verify
 (lambda ((msg <signed-message>) (private <key>) (public <key>))
   (if (= (rsa-transform (signed-message-signature msg) public)
          (compress (signed-message-body msg) private))
       (rsa-decrypt (signed-message-body msg) private)
       #f)))
...
(define crack-rsa
  (lambda ((key <key>))
      (let ((p (smallest-divisor (key-modulus key))))
        (let ((q (/ (key-modulus key) p)))
          (let ((m (* (- p 1)(- q 1))))
            (letrec ((loop
                     (lambda (i)
                       (if (integer? (/ (+ 1 (* m i)) (key-exponent key)))
                           (/ (+ 1 (* m i)) (key-exponent key))
                           (loop (+ 1 i))))))
              (cons (loop 1) (key-modulus key))))))))

(define crack-rsa-2
  (lambda ((key <key>) (base <integer>) (max <integer>))
    (let ((p (pollard (key-modulus key) base max)))
      (if (equal? p #f) #f
          (let ((q (/ (key-modulus key) p)))
            (let ((m (* (- p 1)(- q 1))))
              (letrec ((loop
...
(define pollard
  (lambda ((n <integer>) (base <integer>) (max <integer>))
    (letrec ((loop
              (lambda ((k <integer>) (prev <integer>))
                (cond
                  ((> k max) #f)
                  (else
                  (let ((r (exptmod prev k n)))
                    (let ((d (gcd (- r 1) n)))
                      (cond
                        ((equal? d 1) (loop (+ k 1) r))
                        ((equal? d n) (loop (+ k 1) r))
                        (else
                         d)))))))))
             (loop 1 base))))